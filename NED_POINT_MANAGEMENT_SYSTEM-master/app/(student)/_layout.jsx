...
export default function Layout() {
  ...
  const handleRegisterBus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsBusRegistered(true);
        setRegisteredBusName(selectedBus);
      } else {
  ...
  const cancelBusRegistration = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/cancel-bus-registration/${loggedInUser.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const checkBusAndFeeStatus = async () => {
    ...
    try {
      ...
      const busRes = await fetch(
        `http://localhost:3000/check-bus-status/${loggedInUser.id}`
      );
      ...
      if (!busData.bus_status) {
        ...
        const feeRes = await fetch(
          `http://localhost:3000/fee-status/${loggedInUser.id}`
        );
  ...
  const handlePayFee = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/pay-fee`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.status === 200) {
        // Backend confirmed successful payment update
        setFeeStatus("Completed");
        setModalMessage("Your fee is paid.");
        alert("Your fee is successfully paid.");
      } else if (response.status === 404) {
        // No payment record found or fee already paid
        alert(
          "No pending payment found. Either you have no fee due or it's already been paid."
        );
  ...
  const checkBusAndFeedbackStatus = async () => {
    setLoading(true);
    try {
      // 1. Check bus status and get bus_id
      const busRes = await fetch(
        `http://localhost:3000/check-bus-status/${loggedInUser.id}`
      );
      const busData = await busRes.json();

      if (!busData.bus_status || busData.bus_status === "Not Registered") {
        setModalMessage("Register a bus first");
        setBusRegistered(false);
      ...
      // Get the bus_id for the registered bus
      const busInfoRes = await fetch(
        `http://localhost:3000/get-registered-bus/${loggedInUser.id}`
      );
      const busInfo = await busInfoRes.json();

      if (!busInfo.bus_id) {
        setModalMessage("Error: Could not find your registered bus");
        setBusRegistered(false);
        setFeedbackgiven(null);
        setFeedbackVisible(true);
        return;
      ...
      // Store bus_id for later use
      setBusId(busInfo.bus_id);

      // 2. Check feedback status
      const feedbackRes = await fetch(
        `http://localhost:3000/feedback-status/${loggedInUser.id}`
      );
      const feedbackData = await feedbackRes.json();

      setFeedbackgiven(feedbackData.feedbackGiven);
      setModalMessage(
        feedbackData.feedbackGiven ? "Your feedback is already given" : ""
      );
  ...
  const handlefeedback = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:3000/submit-feedback", {
        ...
        body: JSON.stringify({
          student_id: loggedInUser.id,
          comments: inputFeedback,
          bus_name : registeredBusName
        }),
      });
      ...
      // Check if the request was successful (status 200)
      if (response.status === 200) {
        // Backend confirmed successful feedback submission
        setFeedbackgiven(true);
        setModalMessage("Your feedback is already given");
        setInputFeedback(""); // Clear the input
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/delete-account/${loggedInUser.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      alert(
        "An error occurred while deleting your account. Please check the console for details."
      );
    }
  };
  ...
  return (
    <ThemedView style={{ flex: 1, backgroundColor: Colors.primary }}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.registerModal}>
            ...
            {isBusRegistered && registeredBusName !== "Not Registered" ? (
      ...
      <Modal
        ...
        onRequestClose={() => setPayFeeVisible(false)}
      >
        <View style={styles.modalOverlay}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.feeModal}>
            ...
            {!busRegistered ? (
              ...
              <Text style={styles.paidText}>
                Your feedback is already given
              </Text>
            ) : (
              <>
                <Text style={styles.paidText}>
                ...
                />
                <Pressable
                  style={[styles.payButton, { opacity: loading ? 0.6 : 1 }]}
